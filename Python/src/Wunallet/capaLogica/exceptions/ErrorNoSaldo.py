...
class ErrorNoSaldo(ErrorLogico):
    ...
    def __init__(self, saldo=None, requerimiento=None, considerarLimiteSaldo=False):
        if considerarLimiteSaldo:
            super().__init__(f"Tu pago ha sido rechazado ya que no cuentas con saldo suficiente o tu producto de origen no permite mover el valor indicado.")
        else:
